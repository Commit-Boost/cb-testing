...
- **docs/DESIGN.md** - WHY the repo exists + **the 7 design laws** (each prevents a named smell).
...
- **.agent/NORTH-STAR.md** - the internal staged plan (P0-P5), ratified directions, and open scars behind DESIGN.
...
this way once. Gate on the EXIT CODE, not on matched lines:
```bash
set -o pipefail
...
| a plan shipped | `.agent/plans/INDEX.md` (`live` -> `landed`) |
